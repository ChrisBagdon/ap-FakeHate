...
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/EN/normalized_v1_gs.pickle'  # saving gridsearch
...
v1_df.to_csv('../../training_results/EN/lr_tfidf_results_v1.tsv', sep='\t', index=False)
...
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/EN/normalized_v2_gs.pickle'
pickle.dump(lr_tfidf_v1, open(filename, 'wb'))
v2_df = pd.DataFrame()
v2_df['params'] = lr_tfidf_v1.cv_results_['params']
v2_df['scores'] = lr_tfidf_v1.cv_results_['mean_test_score']
v2_df.to_csv('../../training_results/EN/lr_tfidf_results_v2.tsv', sep='\t', index=False)

...
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/EN/xgb_v11_gs.pickle'  # saving gridsearch
...
v13_df.to_csv('../../training_results/EN/xgb_tfidf_results_v13.tsv', sep='\t', index=False)

grid_search = GridSearchCV(pipeline, parameters, cv=StratifiedKFold(5, shuffle=True, random_state=0), n_jobs=-1)
print(time.strftime("%m/%d/%Y %H:%M:%S"))
xgb_v21 = grid_search.fit(data_v2["Tweets"], data_v2['spreader'])
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/EN/xgb_v21_gs.pickle'  # saving gridsearch
...
v23_df.to_csv('../../training_results/EN/xgb_tfidf_results_v23.tsv', sep='\t', index=False)
...
# xgb_en_v1 = xgb.XGBClassifier(random_state=0, colsample_bytree=0.6, eta=0.01, max_depth=6, n_estimators=300,
#                               subsample=0.8)

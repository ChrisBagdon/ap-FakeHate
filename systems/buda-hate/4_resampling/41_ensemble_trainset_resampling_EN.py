...
data_en_v1 = pd.read_csv(loc_data + 'clean_en_data_v1.tsv', delimiter='\t', encoding='utf-8')
data_en_v2 = pd.read_csv(loc_data + 'clean_en_data_v2.tsv', delimiter='\t', encoding='utf-8')
...
xgb_pl = Pipeline([('vect', TfidfVectorizer(min_df=8, ngram_range=(1, 2), use_idf=True, smooth_idf=True,
                                            sublinear_tf=True)),
                   ('xgb', xgb.XGBClassifier(colsample_bytree=0.7, eta=0.1, max_depth=6, n_estimators=300,
                                             subsample=0.8))])

lr_pl = Pipeline(
    [('vect', TfidfVectorizer(min_df=10, ngram_range=(1, 1), use_idf=True, smooth_idf=True, sublinear_tf=True)),
...
rf_pl = Pipeline(
    [('vect', TfidfVectorizer(min_df=7, ngram_range=(1, 2), use_idf=True, smooth_idf=True, sublinear_tf=True)),
     ('rf', RandomForestClassifier(n_estimators=300, min_samples_leaf=8, criterion='gini'))])

svm_pl = Pipeline(
    [('vect', TfidfVectorizer(ngram_range=(1, 1), min_df=6, sublinear_tf=True, use_idf=True, smooth_idf=True)),
...
for train_index, test_index in cv.split(X, y):
    ...
    # XGB on tweets
    X_train, X_test = data_en_tw_cons.iloc[list(train_index), 2:], data_en_tw_cons.iloc[list(test_index), 2:]
...
for train_index, test_index in cv.split(X, y):
    ...
    # XGB
    X_train, X_test = data_en_v2["Tweets"][train_index], data_en_v1["Tweets"][test_index]
    xgb_pl.fit(X_train, y_train)
    preds["xgb"] = xgb_pl.predict_proba(X_test)[:, 1]

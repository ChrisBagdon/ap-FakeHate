...
es_data_tweet_consist = pd.DataFrame(list(zip([es_data["ID"][i*200] for i in range(int(len(es_data["ID"]) / 200))],
                                              [es_data["spreader"][i*200] for i in
                                               range(int(len(es_data["ID"]) / 200))])), columns=['ID', "spreader"])
...
tweets_szerz = [" ".join(list(es_data["Tweets"])[i*200:199+i*200]) for
                i in range(int(len(len_tw_char) / 200))]

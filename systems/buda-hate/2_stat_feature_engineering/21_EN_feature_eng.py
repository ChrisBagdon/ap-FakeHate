...
en_data.to_csv('../data/en_data_tweetenkent.tsv', sep='\t', index=False)
...
en_data_tweet_consist = pd.DataFrame(list(zip([en_data["ID"][i * 200] for i in range(int(len(en_data["ID"]) / 200))],
                                              [en_data["spreader"][i * 200] for i in
                                               range(int(len(en_data["ID"]) / 200))])), columns=['ID', "spreader"])
...
tweets_szerz = [" ".join(list(en_data["Tweets"])[i * 200:199 + i * 200]) for
                i in range(int(len(len_tw_char) / 200))]
...
en_data_tweet_consist.to_pickle('../data/en_data_tweet_consist.pkl')

...
#########################################################
# load DF-s
result_es = pd.read_csv('../data/es_ensemble_train.tsv', delimiter='\t', encoding='utf-8')
result_es_v2 = pd.read_csv('../data/es_ensemble_dev.tsv', delimiter='\t', encoding='utf-8')
...
params = {'solver': ['saga'],
          'penalty': ['elasticnet'],
          'C': [0.1, 0.2, 0.4, 0.7, 0.9, 1, 1.2, 1.5, 2],
...
print(classification_report(result_es["y_truth"], logreg.predict(result_es.iloc[:, 1:])))
print(confusion_matrix(result_es["y_truth"], logreg.predict(result_es.iloc[:, 1:])))

print(classification_report(result_es_v2["y_truth"], logreg.predict(result_es_v2.iloc[:, 1:])))
print(confusion_matrix(result_es_v2["y_truth"], logreg.predict(result_es_v2.iloc[:, 1:])))

...
#########################################################
# meta
r = open('../../../datasets/pan20-fake-train/en/truth.txt', "r")
data = r.read().split("\n")
idk = []  # id
spreader = []  # yes or no
...
pathlist = Path('../../../datasets/pan20-fake-train/en').glob('**/*.xml')
ids = []
x_raw = []
s = 0
...
en_data_tweet_consist = pd.DataFrame(list(zip([en_data["ID"][i * 100] for i in range(int(len(en_data["ID"]) / 100))],
                                              [en_data["spreader"][i * 100] for i in
                                               range(int(len(en_data["ID"]) / 100))])), columns=['ID', "spreader"])

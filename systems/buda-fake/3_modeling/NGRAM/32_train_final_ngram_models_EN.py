...
# LR
lr_vectorizer_v1 = TfidfVectorizer(ngram_range=(1, 2), min_df=8, sublinear_tf=True, use_idf=True, smooth_idf=True)
...
# SVM
svm_vectorizer_v1 = TfidfVectorizer(ngram_range=(1, 2), min_df=4, sublinear_tf=True, use_idf=True, smooth_idf=True)
...
# RF
rf_vectorizer_v1 = TfidfVectorizer(ngram_range=(1, 2), min_df=10)
...
# XGB
vect_xgb_en_v1 = TfidfVectorizer(min_df=9, ngram_range=(1, 2), use_idf=True, smooth_idf=True, sublinear_tf=True)
...
svm_v1 = svm.SVC(C=1000, kernel='linear',  probability=True, verbose=False)
...
rf_v1 = RandomForestClassifier(n_estimators=400, min_samples_leaf=6, criterion='gini', random_state=0, oob_score=True)
rf_v1.fit(rf_X_v1, data_v1["spreader"])
pickle.dump(rf_v1, open('../../models/en/rf_v2.sav', 'wb'))

# Fitting best XGB
xgb_en_v1 = xgb.XGBClassifier(colsample_bytree=0.7, eta=0.01, max_depth=6, n_estimators=300, subsample=0.6)

...
data_v1 = pd.read_csv('../../data/clean_es_data_v1.tsv', delimiter='\t', encoding='utf-8')
data_v2 = pd.read_csv('../../data/clean_es_data_v2.tsv', delimiter='\t', encoding='utf-8')

pipeline = Pipeline([('vect', TfidfVectorizer(use_idf=True, smooth_idf=True, sublinear_tf=True)),
                     ('xgb', xgb.XGBClassifier(random_state=0, use_label_encoder=False))])
...
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/ES/xgb_v12_gs_es.pickle'  # saving gridsearch
...
v12_df.to_csv('../../training_results/ES/xgb_tfidf_results_v12_es.tsv', sep='\t', index=False)

grid_search = GridSearchCV(pipeline, parameters, cv=StratifiedKFold(5, shuffle=True, random_state=0), n_jobs=-1)
print(time.strftime("%m/%d/%Y %H:%M:%S"))
xgb_v22 = grid_search.fit(data_v2["Tweets"], data_v2['spreader'])
print(time.strftime("%m/%d/%Y %H:%M:%S"))
filename = '../../training_results/ES/xgb_v22_gs.pickle_es'  # saving gridsearch
...
v22_df.to_csv('../../training_results/ES/xgb_tfidf_results_v22_es.tsv', sep='\t', index=False)

...
def Language(input_folder,lang):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        data.drop(['lang', 'class', 'text', 'author_id', 'spreader'], axis=1), data['spreader'], test_size=0.3, random_state=128, shuffle=True)

    model = Model(X_train, X_test, y_train, y_test)

    features = data.drop(['lang', 'class', 'text', 'author_id', 'spreader'], axis=1)

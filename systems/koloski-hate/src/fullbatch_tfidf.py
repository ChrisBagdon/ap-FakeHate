...
def train(X, Y, output=False):
    ...
    trained_models = {}
    for nrep in range(5):
        for nfeat in [5000, 10000, 20000]:
            tokenizer, feature_names, data_matrix = get_features(dataframe, max_num_feat=nfeat)

            reducer = TruncatedSVD(n_components=min(1024, nfeat * len(feature_names) - 1))
            ...
            parameters = {'kernel': ["linear", "poly"], 'C': [0.1, 1, 10, 100, 500], "gamma": ["scale", "auto"],
                          "class_weight": ["balanced", None]}
            svc = svm.SVC()
            clf1 = GridSearchCV(svc, parameters, verbose=0, n_jobs=8, cv=10)
            clf1.fit(X_train, y_train)
            logging.info(
                str(max(clf1.cv_results_['mean_test_score'])) + " training configuration with best score (SVM)")
            ...
            clf2 = GridSearchCV(svc, parameters, verbose=0, n_jobs=8, cv=10)
    ...
    reducer = TruncatedSVD(n_components=min(1024, final_feature_number * len(feature_names) - 1)).fit(data_matrix)
...
def export():
    ...
    with open(os.path.join(config.PATH_MODELS, "tokenizer_en_svm.pkl"), mode='wb') as f:
        pickle.dump(tokenizer, f)
    with open(os.path.join(config.PATH_MODELS, "clf_en_svm.pkl"), mode='wb') as f:
        pickle.dump(clf, f)
    with open(os.path.join(config.PATH_MODELS, "reducer_en_svm.pkl"), mode='wb') as f:
        pickle.dump(reducer, f)
    # ES
    XRaw, YRaw = parse_data.export(config.PATH_DATA_ES, config.PATH_DATA_ES_TRUTH)
    tokenizer, clf, reducer = train(XRaw, YRaw)
    with open(os.path.join(config.PATH_MODELS, "tokenizer_es_svm.pkl"), mode='wb') as f:
        pickle.dump(tokenizer, f)
    with open(os.path.join(config.PATH_MODELS, "clf_es_svm.pkl"), mode='wb') as f:
        pickle.dump(clf, f)
    with open(os.path.join(config.PATH_MODELS, "reducer_es_svm.pkl"), mode='wb') as f:
        pickle.dump(reducer, f)


def _import(lang, path_in=config.PATH_MODELS):
    """Imports tokenizer,clf,reducer from param(path_in, default is ../models)"""
    tokenizer = pickle.load(open(os.path.join(path_in, "tokenizer_" + lang + "_svm.pkl"), 'rb'))
    clf = pickle.load(open(os.path.join(path_in, "clf_" + lang + "_svm.pkl"), 'rb'))
    reducer = pickle.load(open(os.path.join(path_in, "reducer_" + lang + "_svm.pkl"), 'rb'))

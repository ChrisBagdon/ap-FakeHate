...
def get_features(df_data, max_num_feat=1000, labels=None):
    ...
    symbolic_features = [
        ('word', pipeline.Pipeline([('s1', text_col(key='no_stopwords')), ('word_tfidf', tfidf_word_unigram)])),
    ...
    # neural_features = [('neural-doc2vec', pipeline.Pipeline([('s14', text_col(key='no_stopwords')),
    # ('sentence_embedding_mean', sentence_embedder_dm)]))]

...
def train(X, Y, output=False):
    ...
    for nrep in range(5):
        for nfeat in [2500, 5000, 10000, 15000]:
            for dim in [256, 512, 768]:
                ...
                parameters = {"loss": ["hinge", "log"], "penalty": ["elasticnet"],
                              "alpha": [0.01, 0.001, 0.0001, 0.0005],
                              "l1_ratio": [0.05, 0.25, 0.3, 0.6, 0.8, 0.95], "power_t": [0.5, 0.1, 0.9]}
                #                svc = svm.SVC()
                svc = SGDClassifier()
                clf1 = GridSearchCV(svc, parameters, verbose=0, n_jobs=8, cv=10, refit=True)
                clf1.fit(X_train, y_train)
                logging.info(str(max(
                    clf1.cv_results_['mean_test_score'])) + " training configuration with best score (SGD, {})".format(
                    dim))
                ...
                logging.info(str(max(
                    clf2.cv_results_['mean_test_score'])) + " training configuration with best score (LR,{})".format(
                    dim))
                ...
                #     "class_weight":["balanced",None]}
                parameters = {"loss": ["hinge", "log"], "penalty": ["elasticnet"],
                              "alpha": [0.01, 0.001, 0.0001, 0.0005], "l1_ratio": [0.05, 0.25, 0.3, 0.6, 0.8, 0.95],
                              "power_t": [0.5, 0.1, 0.9]}
                #                svc = svm.SVC()
                svc = SGDClassifier()
                clf1 = GridSearchCV(svc, parameters, verbose=0, n_jobs=8, cv=10, refit=True)
                ...
                logging.info("LR 5fCV accuracy: %0.2f (+/- %0.2f)" % (scores.mean(), scores.std() * 2))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(config.PATH_IMGS + "/tfidif-merged-cv-expanded.png", dpi=300)
...
def exportMerged():
    X, Y = parse_data.exportMerged()
    tokenizer, clf, reducer = train(X, Y)
    with open(os.path.join(config.PATH_MODELS, "tokenizer_merged_cv.pkl"), mode='wb') as f:
        pickle.dump(tokenizer, f)
    with open(os.path.join(config.PATH_MODELS, "clf_merged_cv.pkl"), mode='wb') as f:
        pickle.dump(clf, f)
    with open(os.path.join(config.PATH_MODELS, "reducer_merged_cv.pkl"), mode='wb') as f:
...
def _import(lang, path_in=config.PATH_MODELS):
    ...
    tokenizer = pickle.load(open(os.path.join(path_in, "tokenizer_merged_cv.pkl"), 'rb'))
    clf = pickle.load(open(os.path.join(path_in, "clf_merged_cv.pkl"), 'rb'))
    reducer = pickle.load(open(os.path.join(path_in, "reducer_merged_cv.pkl"), 'rb'))
